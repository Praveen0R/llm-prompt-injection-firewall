...
from app.ai_detector import AIPromptAnalyzer
...
class PromptInjectionDetector:
    ...
    def __init__(self):
        self.semantic_analyzer = AIPromptAnalyzer()
        self.rules = [
            {
    ...
    def analyze(self, prompt: str) -> DetectionResult:
        ...
        semantic_score = semantic_result.get("risk", 0)

        if semantic_score > 0:
            score = max(score, semantic_score)

        if semantic_result.get("threat"):
            category = semantic_result.get("category", "SEMANTIC_THREAT")

            if category not in threat_types:
                threat_types.append(category)

            if "semantic_analysis" not in matched_rules:
                matched_rules.append("semantic_analysis")

            reason = semantic_result.get(
                "reason",
