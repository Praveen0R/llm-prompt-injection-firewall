...
class PromptInjectionDetector:
    ...
    def analyze(self, prompt: str) -> DetectionResult:
        ...
        behavior_signals = self._analyze_behavior(normalized)

        for signal in behavior_signals:
            if signal not in explanations:
                explanations.append(signal)

        # Convert behavioral detections into a security result
        if behavior_signals:
    ...
    @staticmethod
    def _analyze_behavior(self, text: str) -> list[str]:
        signals = []

        behavior_patterns = [
            # 1. Direct request for protected information
            (
                r"\b(reveal|show|print|display|give|provide|share|send|tell|"
                r"explain|describe)\b.{0,100}"
                r"\b(system prompt|hidden instructions|internal instructions|"
                r"private instructions|secret instructions|internal program|"
                r"source code|internal logic|implementation details)\b",
                "The prompt attempts to obtain protected internal information."
            ),

            # 2. Requests for internal information using different wording
            (
                r"\b(internal|private|hidden|secret|confidential|protected)\b"
                r".{0,80}"
                r"\b(program|code|logic|instructions|prompt|implementation|"
                r"configuration|details)\b",
                "The prompt attempts to access internal or protected information."
            ),

            # 3. Educational/social-engineering wording combined with protected data
            (
                r"\b(study|educational|education|research|learning|improve myself|"
                r"practice|project|assignment)\b.{0,100}"
                r"\b(internal|private|hidden|secret|system prompt|source code|"
                r"internal program|instructions|logic)\b",
                "The prompt uses a legitimate-sounding purpose to request protected information."
            ),

            # 4. Instruction override
            (
                r"\b(ignore|disregard|forget|override|cancel|replace)\b.{0,100}"
                r"\b(previous|earlier|existing|original)?\s*"
                r"\b(instructions|rules|policy|security|safety|restrictions)\b",
                "The prompt attempts to override an existing instruction or security boundary."
            ),

            # 5. Security bypass
            (
                r"\b(bypass|evade|circumvent|disable|remove|break|avoid)\b.{0,100}"
                r"\b(security|filter|restriction|safety|protection|guardrail|firewall)\b",
                "The prompt attempts to circumvent a security control."
            ),

            # 6. Role manipulation
            (
                r"\b(you are now|act as|pretend to be|roleplay as|"
                r"assume the role|behave like)\b",
                "The prompt attempts to manipulate the AI into adopting a different role."
            ),

            # 7. Concealment / social engineering
            (
                r"\b(do not tell|don't tell|hide this|keep this secret|"
                r"pretend|make it look|do not mention)\b",
